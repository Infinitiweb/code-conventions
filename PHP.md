...
Общие рекомендации к оформлению читать на [официальном сайте](http://docs.phpdoc.org/references/phpdoc/index.html) например для `@var`:

```php
@var [“Type”] [$element_name] [<description>]
```

Комментарии к свойствам и переменным должны распологаться на одной строке.
...
Комментарии к методам и функциям оформляются в несколько строк, как рекомендовано в phpdoc.

Плохо:
```php
/**
 * @param Object $object, array $params
 * @return null|Object
 */
public function someMethod(Object $object, $params = []): ?Object 
{
...
### 📖 Тело условия не должно быть на той же строке, что и само условие

Нельзя прописывать условия в одну строку, так же тело условия должно быть обёрнуто в фигурные скобки

Плохо:
```php
if (count($userProjects) > 0) return $userProjects;
